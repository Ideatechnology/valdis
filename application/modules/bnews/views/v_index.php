...
<div class="row-fluid">
					...
					<div class="span12">
						...
						<?php } ?>
                            ...
                            <br>
                            <form action="<?php echo site_url('bnews/index'); ?>" method="post" name="form1" class="form-horizontal" >
				...
				if (count($query) > 0) {
					...
					foreach($query as $row)
					{
						?>
                            ...
                            <?php if ($this->session->userdata('s_sys_type')!=3) { ?>
                            <a title="" rel="tooltip" class="btn btn-mini btn-danger" href="<?php echo site_url('bnews/submithapus/'.$row['id_news']);?>" onclick="return confirm('Anda Yakin ingin Menghapus?'); "><i class="icon-delete"></i> Delete</a>
							<?php } ?>
                            </div>                        </td>
									  </tr>
