...
class Bpages extends CI_Controller {
		...
		public function index_satuan()
	{
		...
		$sql = "select b.* from  pages b where b.category=2 order by b.id_pages desc";
		$data['query'] = $this->db->query($sql);
		
		$this->template->load('template_backend','v_index_satuan',$data);
	...
	public function index_sejarah()
	{
		...
		$this->template->load('template_backend','v_index_sejarah',$data);
...
return show_error($err->getMessage());
	...
	public function edit()
	{
		...
		$data['pages_desc_eng'] = $row['pages_desc_eng'];
...
try
{
 ...
 'create_author' => $this->session->userdata('s_sys_name'),
			   'pages_desc' => $this->input->post('pages_desc'),
			    'pages_desc_eng' => $this->input->post('pages_desc_eng'),
			   'pages_name' => $this->input->post('pages_name')
			   //,'create_time' => date("Y-m-d H:i:s")
			);
$this->load->helper('uploadut');
