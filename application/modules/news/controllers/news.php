...
class News extends CI_Controller {
	...
	public function index()
	{
		...
		$per_page = 4;  
		$qry = "select * from news where category=1 and publish='Y' order by create_time desc";
	...
	public function arsip()
	{
		...
		$this->load->library('pagination');
		$per_page = 4;  
		$qry = "select * from news where category=1 and publish='Y' and  MONTH(create_time)='".$bulan."' and YEAR(create_time)='".$tahun."' order by create_time  desc";
		...
		$offset = ($this->uri->segment(5) != '' ? $this->uri->segment(5):0);
			...
			$config['base_url']= base_url().'/news/arsip/'.$bulan.'/'.$tahun.'/'; 
			$config['suffix'] = '?'.http_build_query($_GET, '', "&"); 
			$this->pagination->initialize($config);
			$this->data['paginglinks'] = $this->pagination->create_links();    
